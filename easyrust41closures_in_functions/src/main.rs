// You can make your own functions that take closures, 
// but inside them it is less free and you have to decide the type. 
// Outside a function a closure can decide by itself between Fn, FnMut and FnOnce, 
// but inside you have to choose one. 
// The best way to understand is to look at a few function signatures. 
// Here is the one for .all(). We remember that it checks an iterator 
// to see if everything is true (depending on what you decide is true or false).
